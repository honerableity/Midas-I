...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(sub =>
      sub
        .setName('give')
        .setDescription('Give a product to a verified user')
        .addUserOption(opt =>
    ...
    .addSubcommand(sub =>
      sub
        .setName('revoke')
        .setDescription('Revoke a product from a user')
        .addUserOption(opt =>
          opt.setName('user').setDescription('Target user').setRequired(true)
        )
        .addStringOption(opt =>
...
async function handleGive(interaction) {
  ...
  const verifiedRecord = await getVerifiedUser(targetUser.id);
  if (!verifiedRecord) {
    await logCommandActivity(interaction, {
      subcommand: 'give',
      success: false,
      fields: { discordUser: interaction.user, targetUser, productId },
      note: 'Target user is not verified.',
    });
    return interaction.editReply({
      content: `${targetUser} belum verifikasi. Suruh mereka jalankan \`/verify start\` dulu.`,
    });
  }
  ...
  if (userOwnsProduct(product, targetUser.id)) {
    await logCommandActivity(interaction, {
      subcommand: 'give',
      success: false,
      fields: { discordUser: interaction.user, targetUser, productId, productName: product.name },
      note: 'Target user already owns this product.',
    });
    return interaction.editReply({
      content: `${targetUser} sudah punya produk **${product.name}**.`,
    });
  }

  try {
    await giveProductToUser(productId, targetUser.id);
  } catch (err) {
    console.error('Failed to give product (Firestore write failed):', err);
    await logCommandActivity(interaction, {
      subcommand: 'give',
      success: false,
      fields: { discordUser: interaction.user, targetUser, productId, productName: product.name },
  ...
  return interaction.editReply({
    content: `Produk **${product.name}** berhasil diberikan ke ${targetUser}.`,
  });
}
...
async function handleRevoke(interaction) {
  ...
  if (!userOwnsProduct(product, targetUser.id)) {
    ...
    return interaction.editReply({
      content: `${targetUser} belum punya produk **${product.name}**.`,
    });
  }

  try {
    await revokeProductFromUser(productId, targetUser.id);
  } catch (err) {
    console.error('Failed to revoke product (Firestore write failed):', err);
    await logCommandActivity(interaction, {
      subcommand: 'revoke',
      success: false,
      fields: { discordUser: interaction.user, targetUser, productId, productName: product.name },
